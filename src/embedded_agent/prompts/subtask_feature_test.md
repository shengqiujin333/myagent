...
3. **总体设计 (Overall Design)**：系统级架构、模块划分与接口契约（用于理解全局数据流）。
...
5. **子任务详细设计 JSON 集合**：上一步产出的所有子任务文件。

**🚨 绝对铁律（触碰即判定为劣质输出，直接终止）：**
1. **每个子任务一个独立测试 JSON 文件**：你的输出**必须是多个独立的 JSON 代码块，文件名字为对应的设计文件序号加T，比如设计方案文件是1D，测试方案文件则是1T**。每个代码块代表一个子任务的测试方案文件。严禁将多个任务的测试方案合并！
...
## 2. 独立测试 JSON 文件输出契约 (Standalone Test JSON Contract)

**【输出格式要求】**：请按照子任务的 `execution_order` 顺序，依次输出多个独立的 JSON 代码块。每个代码块必须用 ` ```json ` 包裹，并在上方用注释标明文件名（如 `<!-- File: test_T1.1.json -->`）。
