...
1. **统一总 JSON 输出**：你的输出必须是一个完整、合法的 JSON 对象，所有子任务放在顶层 `subtasks` 数组中。每个数组元素代表一个子任务。不要输出多个 JSON 代码块、文件注释或数组之外的额外内容。
2. **只管“事”与“设计”，严禁越权**：
...
**【输出格式要求】**：只输出一个完整、合法的 JSON 对象。顶层必须包含 `subtasks` 数组，数组中的每个元素必须是一个合法的子任务设计对象。
**【输出内容要求】**：按照 `execution_order` 排列所有子任务。Python 后续会根据数组顺序、`execution_order` 和 `task_id` 将每个元素分别落盘，因此你不需要输出文件名、文件注释或多个代码块。
**【无子任务要求】**：如果根据输入没有需要拆分的子任务，输出 `{"subtasks": []}`。
  ...
  "subtasks": [
    {
      ...
      "execution_order": 1,
      "dependencies": [],
      "design_objective": "完成 MCU 时钟树配置，并实现阻塞式的 UART 轮询发送接口。",
      "detailed_design_scope": {},
      "compatibility_strategy": "说明与已有功能的兼容和隔离方案。"
    }
  ]
}
```
